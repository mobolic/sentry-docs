...
Generally speaking, Apdex is represented with the inclusion of the target response time. For example, `apdex(300)` is the Apdex score with a target response time of 300 milliseconds.
